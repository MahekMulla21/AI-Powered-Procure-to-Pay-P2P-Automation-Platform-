...
try:
    ...
    for p in [str(DECISION_AGENT_DIR), str(DECISION_AGENT_DIR / "structured_data")]:
        if p not in sys.path:
            sys.path.insert(0, p)
...
def run_invoice_decision_pipeline(file_path: str, file_id: int):
    ...
    try:
        ...
        from invoice_decision_runner import run_invoice_decision
        result = run_invoice_decision(file_path, file_id)
        logger.info(f"[INVOICE RESULT] {result}")
        ...
        _invoice_decisions[file_id] = {
            ...
            "verdict": {
                ...
                "summary":    verdict_data.get("summary",    "Invoice processed successfully"),
        ...
        logger.info(f"[INVOICE DECISION DONE] file_id={file_id}")
